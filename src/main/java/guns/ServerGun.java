...
public class ServerGun extends CommonGun {
	...
	public ServerGun(EnumHand hand, EntityPlayerMP player) {
		super(hand);
		owner = player;
	}
	...
	public boolean updateTag(NBTTagCompound gunTag) {
		...
		boolean magChange = gun != null && gunTag != null && !HideNBT.getGunLoadedMagazines(gunTag).magEquals(HideNBT.getGunLoadedMagazines(gun));
		boolean res = super.updateTag(gunTag);

		if (!res && magChange) {
			//		HideNBT.setGunLoadedMagazines(gun, magazine);
			magazine = HideNBT.getGunLoadedMagazines(gun);
			PacketHandler.INSTANCE.sendTo(new PacketSyncMag(magazine, hand), owner);
		}
		return res;
	}
	...
	protected void updateData() {
		...
		if (isGun()) {
			...
			int shootdelay = HideNBT.getGunShootDelay(gun);
			if (shootdelay != 0)
				lastShootTime = System.currentTimeMillis() - RPMtoMillis(modifyData.RPM) + shootdelay;
		}
		stopReload();
	}
	...
	public void tickUpdate() {
		...
		//ShootDelayが残っていれば
		if (lastShootTime != 0) {
			int shootdelay = (int) (RPMtoMillis(modifyData.RPM) - (System.currentTimeMillis() - lastShootTime));
			//残りが0になったら消しとく
			if (0 < shootdelay) {
				HideNBT.setGunShootDelay(gun, shootdelay);
			} else {
				HideNBT.setGunShootDelay(gun, 0);
				lastShootTime = 0;
			}
		}
		//TODO
