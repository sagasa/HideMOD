...
public class EntityBullet extends Entity implements IEntityAdditionalSpawnData {
	...
	public EntityBullet(GunData gun, BulletData bullet, Entity shooter, boolean isADS, float offset, double x, double y,
		...
		// データから読み取る
		float speed = gunData.BULLET_SPEED;
	...
	private void ServerUpdate() {
		...
		Iterator<Hit> HitEntities = RayTracer.getHitEntity(this, world, lvo, lvend).iterator();
		// System.out.println(bulletPower);
		while (HitEntities.hasNext() && bulletPower > 0) {
			Hit hit = HitEntities.next();
