	...
	private static void writeGunInfo(int x, int y) {
		if(PlayerHandler.loadedMagazines!=null){
			...
			if(PlayerHandler.fireMode!=null){
				...
				//射撃モードを描画
				mc.fontRenderer.drawString(PlayerHandler.fireMode.toString().toUpperCase(), x+40, y+39, 0xFFFFFF);
				//残弾
				float fontSize = 1.8f;
				GlStateManager.scale(fontSize, fontSize, fontSize);
				mc.fontRenderer.drawString(LoadedMagazine.getLoadedNum(PlayerHandler.loadedMagazines)+"/"+PlayerHandler.getCanLoadMagazineNum(Minecraft.getMinecraft().player), (x+5)/fontSize, (y+21)/fontSize, 0xFFFFFF, false);
				GlStateManager.scale(1/fontSize, 1/fontSize, 1/fontSize);
				//使用する弾
				mc.fontRenderer.drawString(ItemMagazine.getBulletData(PlayerHandler.UsingBulletName).ITEM_INFO.NAME_DISPLAY, x+40, y+50, 0xFFFFFF);
