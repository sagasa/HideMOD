...
public class BulletData extends DataBase {
	...
	public enum BulletDataList implements EnumDataList {
		...
		/** 通過音 : Sound型 **/
		SOUND_PASSING(null, null, new Sound("sample", 8), DataType.Sound, 21),
	...
	public void setDomain(String Domain) {
		...
		// 音のドメインがなければ定義
		Sound entity = (Sound) getDataObject(BulletDataList.SOUND_HIT_ENTITY);
		if (!entity.name.contains(":")) {
			entity.name = HideMod.MOD_ID + ":" + entity.name;
		}
		Sound groud = (Sound) getDataObject(BulletDataList.SOUND_HIT_GROUND);
		if (!groud.name.contains(":")) {
			groud.name = HideMod.MOD_ID + ":" + groud.name;
		}
		Sound pass = (Sound) getDataObject(BulletDataList.SOUND_HIT_GROUND);
		if (!pass.name.contains(":")) {
			pass.name = HideMod.MOD_ID + ":" + pass.name;
		}
	}
}
